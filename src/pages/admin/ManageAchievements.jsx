...
const ManageAchievements = () => {
  ...
  return (
    <div className="h-screen bg-[#f8f7f4] font-sans flex flex-col overflow-hidden">
      {/* Top Navbar */}
      <header className="bg-stone-900 border-b border-stone-800 shrink-0">
        <div className="w-full px-6 py-0 flex items-stretch justify-between h-14">
          <button
      ...
      {/* Page Layout */}
      <div className="flex-1 overflow-y-auto px-6 py-6 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-stone-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb:hover]:bg-stone-400">

        {/* Toolbar */}
        <div className="flex items-center gap-4 mb-5">
          <div className="shrink-0">
            <h1 className="text-lg font-bold text-stone-900 leading-tight">Achievements</h1>
            <p className="text-xs text-stone-400 mt-0.5">{achievements.length} data tersimpan</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
                  ...
                  {filtered.map((item) => (
                    <tr key={item.id} className="hover:bg-stone-50/70 transition-colors group">
                      ...
                      <td className="px-5 py-3">
                        ...
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noreferrer" className="text-xs text-sky-500 hover:text-sky-700 transition-colors mt-0.5 block truncate max-w-xs">
                            {item.link}
                          </a>
                        )}
                      </td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(item)} title="Edit" className="p-1.5 text-stone-500 hover:text-sky-600 hover:bg-sky-50 rounded-lg transition-colors">
                            <Pencil size={14} />
                          </button>
                          <button onClick={() => handleDelete(item.id)} disabled={isDeleting === item.id} title="Hapus" className="p-1.5 text-stone-500 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors disabled:opacity-40">
                            {isDeleting === item.id ? (
          ...
          {filtered.length > 0 && (
            <div className="px-5 py-3 bg-stone-50 border-t border-stone-100 flex items-center justify-between">
              <span className="text-xs text-stone-400">Menampilkan {filtered.length} dari {achievements.length} data</span>
              {filterCategory !== "all" && (
                <button onClick={() => setFilterCategory("all")} className="text-xs text-stone-500 hover:text-stone-800 transition-colors underline underline-offset-2">
                  Reset filter
                </button>
              )}
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          {/*
            Modal wrapper: flex column, fixed max height, NO overflow here.
            Header + Footer are sticky; only the Body scrolls.
          */}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4
              ...
              <div>
                ...
                <p className="text-xs text-stone-400 mt-1.5">
                  Atau isi URL gambar jika sudah tersimpan di folder public.
                </p>
                <input
                  type="text"
                  placeholder="/certs/nama-file.jpg"
                  value={form.img}
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Nama organisasi / lembaga"
                  className={inputCls}
                />
              </div>
