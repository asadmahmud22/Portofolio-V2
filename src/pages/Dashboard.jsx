      ...
      <div className="px-3 md:px-1 lg:px-1 py-1 md:py-1 lg:py-1 max-w-4xl overflow-y-hidden mx-auto">
        ...
        <div className="mb-4 md:mb-8 text-center">
          ...
          <p className="mb-6 text-gray-600 text-center">
            Dibangun dengan rute API Next.js yang diimplementasikan sebagai fungsi serverless.
          </p>
        </div>

        {/* GitHub Contributions Section */}
        <div className="bg-gray-100 rounded-md md:rounded-xl border border-gray-300 p-3 md:p-6 lg:p-8">
          {/* Section Header */}
          <div className="mb-3 md:mb-6">
            <div className="flex items-center gap-1.5 mb-1">
              <svg
                className="w-3 h-3 md:w-5 md:h-5 text-gray-700"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <p className="text-xs text-gray-400 mt-0.5">@asadmahmud22</p>
          </div>

          {/* Stats Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-4">
            {["Total", "This week", "Best", "Avg"].map((label, i) => (
              <div
                key={i}
                className="bg-white rounded border border-gray-300 p-2 md:p-4 text-center shadow-sm"
              >
                {isLoading ? (
                  <div className="animate-pulse h-6 md:h-8 bg-gray-200 rounded"></div>
                ) : (
                  <p className="text-green-600 text-base md:text-xl lg:text-2xl font-bold">
          ...
          <div className="relative">
            <div className="hidden md:flex justify-between text-xs text-gray-500 mb-2 px-2">
              {[
                ...
                <span key={month} className={index % 2 === 0 ? "" : "hidden lg:inline"}>
                  {month}
                </span>
              ))}
            </div>
            ...
            <div className="overflow-x-auto">
              {isLoading ? (
                <div
                  ...
                  style={{ gridTemplateColumns: "repeat(53, 1fr)", minWidth: "260px", height: "112px" }}
                >
                  {Array.from({ length: 371 }).map((_, index) => (
                    <div key={index} className="w-1.5 h-1.5 md:w-3 md:h-3 lg:w-4 lg:h-4 rounded-sm bg-gray-200" />
                  ))}
                </div>
              ) : hasError ? (
                ...
                <div
                  ...
                  style={{ gridTemplateColumns: "repeat(53, 1fr)", minWidth: "260px" }}
                >
                  {contributions.slice(0, 371).map((contribution, index) => (
                    <div
                      ...
                      className={`w-1.5 h-1.5 md:w-3 md:h-3 lg:w-4 lg:h-4 rounded-sm ${getColorClass(
                        contribution.level
                      )} hover:ring-1 hover:ring-green-500 transition-all duration-200`}
        ...
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          ...
          <div className="bg-gray-100 rounded-lg border border-gray-300 p-4">
            <h3 className="text-sm md:text-base font-semibold mb-3 text-green-600">
              Recent Activity
            </h3>
            <div className="space-y-2 text-gray-700 text-xs md:text-sm">
              <div><span className="text-gray-500">2 hours ago</span> - Committed to project-name</div>
              <div><span className="text-gray-500">1 day ago</span> - Merged pull request</div>
              <div><span className="text-gray-500">3 days ago</span> - Created new repository</div>
            </div>
          </div>

          {/* Quick Stats */}
          <div className="bg-gray-100 rounded-lg border border-gray-300 p-4">
            <h3 className="text-sm md:text-base font-semibold mb-3 text-blue-600">
              Quick Stats
            </h3>
            <div className="space-y-2">
              <div className="flex justify-between text-xs md:text-sm text-gray-600">
                <span>Repositories</span><span className="font-medium text-black">42</span>
              </div>
              <div className="flex justify-between text-xs md:text-sm text-gray-600">
                <span>Stars Earned</span><span className="font-medium text-black">156</span>
              </div>
              <div className="flex justify-between text-xs md:text-sm text-gray-600">
                <span>Followers</span><span className="font-medium text-black">89</span>
              </div>
            </div>
          </div>

          {/* Top Languages */}
          <div className="bg-gray-100 rounded-lg border border-gray-300 p-4 md:col-span-2 lg:col-span-1">
            <h3 className="text-sm md:text-base font-semibold mb-3 text-purple-600">
              Top Languages
            </h3>
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-xs md:text-sm mb-1 text-gray-600">
                  <span>JavaScript</span><span>45%</span>
                </div>
